...
def _prepare_global_state_for_mixer(base_global_state_entities_batch, # List (batch_size) of List of raw entities (current, etc.)
    ...
    This assumes global_state_entities_batch contains placeholders for agent CNN features
    or that such features are appended. For TransfQMix, agent states (including their F_cnn)
    are often part of the global state representation.

    Simplification: For this version, we assume the global state definition in env.py already
    includes entities for each agent that incorporate their F_cnn features.
    The `agent_f_cnn_features_batch` will be used to fill these specific entities.
    This function would identify those agent-specific entities in `base_global_state_entities_batch`
    and update their F_cnn part.

    A more direct approach inspired by some TransfQMix versions:
    The mixer's transformer processes:
    1. Projected agent hidden states (h_t^a)
    2. Embedded non-agent-specific global entities (e.g., current vector)
    3. (Optionally) Embedded F_cnn_a from each agent, if not already implicitly captured by h_t^a
       or if the mixer needs explicit F_cnn features separate from h_t^a.

    The `env._get_observations_and_state()` constructs global state entities where each agent's
    entity includes a placeholder for its F_cnn. We need to replace that.
    """
    batch_size = agent_f_cnn_features_batch.shape[0]
    num_agents = agent_f_cnn_features_batch.shape[1]
    
    # max_global_entities expected by mixer_nn (from global_state_spec)
    # This function will return a list (batch_size) of list (entities) of Tensors for the mixer
    
    # This logic needs to be very robust based on how env.py structures global state.
    # Assumption: env.py's global state list has first `num_agents` entities as agent representations,
    # and these agent representations have their F_cnn part that needs updating.
    # The `global_entity_raw_feature_dim` is the padded length.
    ...
    for b_idx in range(batch_size):
        ...
        processed_entities_for_b = []
        
        # Identify agent entities in global state (e.g., first num_agents entities)
        # and update their F_cnn component. Other entities (like current) are passed as is.
        # This is highly dependent on env._get_observations_and_state()'s global state structure.
        
        # For each entity in the raw global state for this batch item:
        num_global_entities_from_env = len(current_b_global_entities_raw)
        
        # Example: if global state from env is [agent1_gs_placeholder, agent2_gs_placeholder, ..., current_entity]
        # And agent_gs_placeholder is [pos_r, pos_c, one_hot_h, DUMMY_F_CNN_FEATURES, flags...]
        # We need to find DUMMY_F_CNN_FEATURES and replace it.
        
        # Let's assume global_state_spec gives enough info, e.g., index of F_cnn features within an agent's global entity
        # This is complex. A simpler way for TransfQMix is often:
        # Mixer input = [projected_h_agent1, ..., projected_h_agentN, embedded_env_current, embedded_global_belief_summary]
        # Where F_cnn is already part of h_agent (as agent transformer processed it).
        # If F_cnn needs to be an *explicit separate input* to the mixer transformer *in addition* to h_states,
        # then agent_f_cnn_features_batch would be embedded and appended.

        # Let's follow the paper's spirit: mixer processes agent states (h_t^a) and global state S_t.
        # S_t from env already includes agent positions/headings AND placeholders for F_cnn.
        # So, we update those placeholders.
        
        temp_entities_for_b = []
        # First num_agents entities are assumed to be agent-specific global representations
        for entity_idx in range(num_global_entities_from_env):
            ...
            if entity_idx < num_agents: # This is an agent's global state entity
                # Assume F_cnn features are located after pos, heading, and before flags
                # This needs precise indexing from env.py's global state construction.
                # Example: [norm_r, norm_c, one_hot_h (8), F_cnn (64), IS_AGENT, IS_ENV, IS_GLOBAL_BELIEF]
                # F_cnn starts at index 2 + num_headings
                # This is fragile. A better way: global_state_spec should define structure.
                start_idx_fcnn = 2 + config_env.get("NUM_HEADINGS", 8) 
                end_idx_fcnn = start_idx_fcnn + agent_f_cnn_features_batch.shape[2] # cnn_dim
                
                # Check if this entity is indeed an agent entity based on flags
                # E.g., IS_AGENT_FLAG == 1. This would make it more robust.
                # For now, relying on order.
                
                # Ensure indices are within bounds of entity_tensor
                if end_idx_fcnn <= len(entity_tensor):
                     entity_tensor[start_idx_fcnn:end_idx_fcnn] = agent_f_cnn_features_batch[b_idx, entity_idx, :]
                else:
                    # This indicates a mismatch in assumption about global state structure or F_cnn dim
                    # For safety, don't modify if indices are wrong, or raise error.
                    # print(f"Warning: F_cnn indices out of bounds for global state entity. Check global state structure.")
...
def train(config):
    ...
    if config['use_cuda'] and torch.cuda.is_available():
        ...
        logger_init = setup_logger("InitialSetup", config['logging']['log_dir'], "INFO")[0] # Temp logger for this
        logger_init.info("Using CUDA")
    else:
        device = torch.device("cpu")
        logger_init = setup_logger("InitialSetup", config['logging']['log_dir'], "INFO")[0]
        logger_init.info("Using CPU")
    ...
    with open(os.path.join(log_subdir, 'experiment_config.yaml'), 'w') as f:
        yaml.dump(config, f, default_flow_style=False)
    with open(os.path.join(tb_writer_subdir, 'experiment_config.yaml'), 'w') as f:
        yaml.dump(config, f, default_flow_style=False)
    ...
    global_state_spec_env = env.get_observation_spec() # Using this as basis for mixer's state spec
    ...
    ) # Added tb_writer_subdir for unique paths per run
    ...
    visualizer = EpisodeVisualizer(
        ...
        cell_size_m=config['environment']['CELL_SIZE_METERS'],
    ...
    for episode_num in range(1, config['training']['num_training_episodes'] + 1):
        ...
        agent_obs_dict, global_state_entities_np = env.reset() # global_state_entities_np is list of np arrays
        ...
        for step_in_ep in range(config['environment']['MAX_STEPS_PER_EPISODE']):
            ...
            actions_dict = {}; agent_q_values_for_action = {}; agent_h_states_current = {}
            
            # Belief maps for each agent (already np arrays from env)
            current_agent_belief_maps_dict_np = {aid: env.agent_belief_maps[aid]['belief'].copy() for aid in agent_ids} # Important: copy for buffer
            ...
            next_agent_belief_maps_dict_np = {aid: env.agent_belief_maps[aid]['belief'].copy() for aid in agent_ids} # Get after env.step
            ...
            if episode_num > config['training']['learning_starts_episodes'] and \
               len(replay_buffer) >= config['training']['batch_size']:
                ...
                for agent_idx in range(num_agents):
                    belief_map_tensor_agent = agent_belief_maps_b[:, agent_idx, :, :] # (batch, r, c)
                    h_in_tensor_agent = h_in_prev_b[:, agent_idx, :]
                    
                    # agent_obs_b is List (batch_size) of List (num_agents) of List (entities)
                    # Need List (batch_size) of List (entities) for this agent_idx
                    current_agent_obs_entities_for_batch = [agent_obs_b[b_idx][agent_idx] for b_idx in range(config['training']['batch_size'])]

                    q_all_actions_agent, _, f_cnn_agent = agent_policy_nn(
                        belief_map_tensor_agent, current_agent_obs_entities_for_batch, h_in_tensor_agent)
                ...
                # Prepare global state for policy mixer
                # global_state_b_raw_from_buffer is List (batch) of List (entities) of np.arrays
                global_state_for_policy_mixer = _prepare_global_state_for_mixer(
                    global_state_b_raw_from_buffer, f_cnn_batch_policy, global_state_spec_env, config['environment'], device
                )
                q_total_policy = mixer_policy_nn(chosen_action_q_vals_batch, global_state_for_policy_mixer, h_out_curr_b)

                # --- Q_tot_target Calculation ---
                with torch.no_grad():
                    ...
                    h_next_target_batch = torch.zeros_like(h_out_curr_b) # Store h output from target_agent_nn

                    for agent_idx in range(num_agents):
                        next_belief_map_tensor_agent = next_agent_belief_maps_b[:, agent_idx, :, :]
                        h_in_for_next_tensor_agent = h_out_curr_b[:, agent_idx, :] # h_out of policy net is h_in for next step
                        
                        current_next_agent_obs_entities_for_batch = [next_agent_obs_b[b_idx][agent_idx] for b_idx in range(config['training']['batch_size'])]

                        q_all_next_actions_target, h_out_target_agent, f_cnn_next_target_agent = agent_target_nn(
                            next_belief_map_tensor_agent, current_next_agent_obs_entities_for_batch, h_in_for_next_tensor_agent)
                        ...
                        h_next_target_batch[:, agent_idx, :] = h_out_target_agent # Collect h from target net

                    # Prepare global state for target mixer
                    global_state_for_target_mixer = _prepare_global_state_for_mixer(
                        next_global_state_b_raw_from_buffer, f_cnn_batch_target, global_state_spec_env, config['environment'], device
                    )
                    # Mixer target uses h_next_target_batch (output from agent_target_nn)
                ...
                tb_writer.add_scalar("Training/Epsilon", epsilon, total_env_steps)
                tb_writer.add_scalar("Training/Mean_Q_total_policy", q_total_policy.mean().item(), total_env_steps)

            if team_done: break
        
        # --- Logging, Target Update, Evaluation, Saving (largely same as before, ensure paths are correct) ---
        avg_iou_ep = ep_iou_sum / ep_steps if ep_steps > 0 else 0
        logger.info(f"Ep: {episode_num}/{config['training']['num_training_episodes']} | Steps: {ep_steps} | Total Steps: {total_env_steps} | Reward: {ep_reward:.2f} | Avg IoU: {avg_iou_ep:.3f} | Epsilon: {epsilon:.3f} | Buffer: {len(replay_buffer)}")
        tb_writer.add_scalar("Episode/Reward", ep_reward, episode_num)
        tb_writer.add_scalar("Episode/Avg_IoU", avg_iou_ep, episode_num)
        tb_writer.add_scalar("Episode/Length", ep_steps, episode_num)

        if episode_num % config['training']['target_update_interval_episodes'] == 0:
            logger.info("Updating target networks...")
            agent_target_nn.load_state_dict(agent_policy_nn.state_dict())
            mixer_target_nn.load_state_dict(mixer_policy_nn.state_dict())

        if episode_num % config['training']['evaluation_interval_episodes'] == 0:
            logger.info(f"--- Running Evaluation after Episode {episode_num} ---")
            total_eval_reward = 0; total_eval_iou = 0; total_eval_steps = 0
            ...
            for eval_ep_num in range(config['training']['num_evaluation_episodes']):
                ...
                obs_eval, gs_eval_np = eval_env.reset() # gs_eval_np is list of np arrays
                ...
                for _ in range(config['environment']['MAX_STEPS_PER_EPISODE']):
                    eval_ep_steps +=1; actions_eval = {}; current_h_eval = {}
            ...
            tb_writer.add_scalar("Evaluation/Avg_Reward", avg_eval_reward, total_env_steps) # Log vs total_env_steps
            ...
            if avg_eval_iou > best_eval_iou:
                ...
                save_checkpoint({ 'episode': episode_num, 'total_env_steps': total_env_steps, 'best_eval_iou': best_eval_iou,
        ...
        if episode_num % config['training']['save_model_interval_episodes'] == 0:
            logger.info(f"Saving periodic model checkpoint at episode {episode_num}.")
            model_save_dir = os.path.join(config['logging']['model_save_dir'], config['experiment_name'], tb_writer_subdir.split('/')[-1])
            save_checkpoint({ 'episode': episode_num, 'total_env_steps': total_env_steps, 
                              'current_eval_iou': avg_eval_iou if 'avg_eval_iou' in locals() else -1,
                'agent_nn_state_dict': agent_policy_nn.state_dict(), 'mixer_nn_state_dict': mixer_policy_nn.state_dict(),
                'agent_target_nn_state_dict': agent_target_nn.state_dict(), 'mixer_target_nn_state_dict': mixer_target_nn.state_dict(),
...
if __name__ == "__main__":
    ...
    except FileNotFoundError: print(f"ERROR: Config file not found: {args.config}"); exit(1)
    except yaml.YAMLError as e: print(f"ERROR: YAML parse error in {args.config}: {e}"); exit(1)
    
    # Ensure log/model directories specific to this run exist
    # (tb_writer_subdir will be unique due to timestamp)
    # The setup_logger and get_tensorboard_writer will create their specific subdirs.
    # We only need to ensure the base parent directories exist.
    ...
    # GIF output parent dir; specific run dir created by visualizer using tb_writer_subdir
